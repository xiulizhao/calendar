(function (root, factory) {
  ...
  var loadDependence = function (fncallback) {
    ...
    if (!window.wwload.calendar) {
      window.wwload.calendar = "wait";
      // requirejs.config({
      //   paths: {
      //     "插件名": "插件路径" // 示例: libs/qrcodejs/qrcode
      //   }
      // });
      require(["libs/mobiscroll/mobiscroll.custom-3.0.0-beta6.min", "css!libs/mobiscroll/mobiscroll.custom-3.0.0-beta6.min"], function () {
        window.wwload.calendar = true;
        replace();
        fncallback();
      });
    } else if (window.wwload.calendar === "wait") {
      setTimeout(function () {
  ...
  function calendar($ele) {
    var config = {};
    config.calendarid = $ele.attr("data-calendarid") || "demo";
    config.theme = $ele.attr("data-theme") || "ios";  //mobiscroll
    config.select = $ele.attr("data-select") || "single"; //多选 multiple
    config.dateFormat = $ele.attr("data-dateformat") || "yy-mm-dd";
    config.months = eval($ele.attr("data-months") || "1");
    config.cancelText = $ele.attr("data-canceltext") || "取消";
    config.setText = $ele.attr("data-settext") || "确定";
    config.display = $ele.attr("data-display") || 'inline';   //bubble
    if (config.select == "multiple") {
      config.nows = eval($ele.attr("data--nows") || ''); //[new Date("2019, 7, 31"),new Date("2019, 7, 29"),new Date("2019, 7, 30")]
    } else {
      config.nows = eval($ele.attr("data--nows") || '');  //new Date("2019, 7, 31")
    }

    var currYear = (new Date()).getFullYear();
    $ele.find(".calendar").attr("id", config.calendarid);
    $ele.find('#' + config.calendarid).mobiscroll().calendar({
      //var mobiscrollInstance = mobiscroll.calendar('#' + config.calendarid, {
      theme: config.theme,          //主题
      lang: 'zh',                  //中文
      select: config.select,           //多选
      dateFormat: config.dateFormat,    //选中日期格式
      months: config.months,                   //多选月
      cancelText: config.cancelText,// 取消按钮的text
      setText: config.setText,// 确认按钮的text
      startYear: currYear - 50,
      endYear: currYear + 50,
      display: config.display,        //
      defaultValue: config.nows,
      controls: ['calendar'],
      onShow: function (event, inst) {

        $.wwclass.helper.updateProp($ele, "data-x-valuetext", event.valueText);
      },
  ...
  var evtInHandler = function ($ele, attribute, value) {
    switch (attribute) {
      case "data--nows":
        ...
        $('#' + config.calendarid).mobiscroll('setVal', config.nows);
